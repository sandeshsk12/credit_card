...
class pred_validation():
    ...
    def pred_validation(self):
        ...
        try:
            ...
            self.logger.log(
                self.log_file, "Breaking the csv file into smaller chunks")
            self.file_operator.break_file_into_smaller_files()
            self.logger.log(
                self.log_file, "Completed Breaking the csv file into smaller chunks")
            ...
            self.logger.log(
                self.log_file, "Checking if any column has an empty column")
            self.file_operator.check_file_contents()
            self.logger.log(
                self.log_file, "Files with empty columns moved into archive")
            self.logger.log(
                self.log_file, "Checking and imputing NULL where data is missing")
            self.file_operator.impute_null()
            self.logger.log(
                self.log_file, "Imputed empty spaces with NULL successfully")
            self.logger.log(self.log_file, "Checking for file name violation")
            self.file_operator.check_filename_and_move()
            self.logger.log(
                self.log_file, "Checking for file name violation complete")
            ...
            self.logger.log(
                self.log_file, "Establishing connection with cassandra database")
            self.database_operator.cassandra_connection()
            self.logger.log(
                self.log_file, "Successfully connected to the database")

            self.logger.log(
                self.log_file, "Creating table in cassandra database")
            self.database_operator.create_prediction_table()
            self.logger.log(
                self.log_file, "Successfully created a table in the database")

            self.logger.log(
                self.log_file, "Inserting values into table in cassandra database")
            self.database_operator.insert_values_into_prediction_database()
            self.logger.log(
                self.log_file, "Successfully inserted data into table in the database")

            self.logger.log(
                self.log_file, "Exporting contents in cassandra database into a csv")
            self.database_operator.db_to_csv(process='prediction')
            self.logger.log(
                self.log_file, "Successfully exported data in the database")

            self.logger.log(
                self.log_file, "closing cassandra database connection")
